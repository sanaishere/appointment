...
export class SalaryService {
  ...
  async evaluate({ month, year }, staff: User) {
    if (!staff.roles.includes(UserRole.STAFF)) {
      throw new HttpException(
        'you are not staff to get salary',
  ...
  async create(month: number, staff: User, year: number) {
    const getRate = (await this.salaryRateService.getPerYear(year))
      .incomePerPatient;
    const appointments = await this.appointmentService.getByStaffIdMonthly({
      month,
      year,
      staffId: staff._id,
    });
  ...
  async increaseSalary(salary: Salary, staff: User) {
    const getRate = (await this.salaryRateService.getPerYear(salary.year))
      .incomePerPatient;
    const appointments = await this.appointmentService.getByStaffIdMonthly({
      month: salary.month,
      year: salary.year,
