...
export class SalaryRateService {
    constructor(@InjectModel(SalaryRate.name) private salatyRateModel:Model<SalaryRate>){}
     async create(input:SalaryRateInput) {
       const found=await this.salatyRateModel.findOne({year:input.year})
       if(found){
        throw new HttpException('you have already insert this year rate',HttpStatus.BAD_REQUEST)
       }
      const createRate=await this.salatyRateModel.create({...input,incomePerPatient:input.income})
     ...
     async update(id:number,input:SalaryRateUpdate) {
       ...
       if(!found){
        throw new HttpException(`salary rate with  id ${id} is not found`,HttpStatus.NOT_FOUND)
       }
     ...
     async getPerYear(year:number) {
        ...
        if(!found){
         throw new HttpException(`year ${year} salary rate is not yet created`,HttpStatus.BAD_REQUEST)
        }
        return found
     }
