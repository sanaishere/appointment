...
export class AppointmentService {
    ...
    try {
      if (appointment) {
        throw new HttpException(
          'you already have appointment at this date,time',
      ...
      if (staffsFreeNow.length === 0) {
        throw new HttpException(
          'there are no staffs free at this time',
  ...
  async update(
    ...
    session.startTransaction();
    try {
      const staffsFree = await this.getFreeStaffs({
        date: appointment.date,
        hours: appointment.startTime,
      });
      if (!staffsFree.includes(staff)) {
        throw new HttpException(
          'this staff would be busy on this time',
          HttpStatus.BAD_REQUEST,
        );
      }
  ...
  async findById(id: string): Promise<Appointment> {
    ...
    if (!appointment) {
      throw new HttpException(
        `there is no appointment with this id ${id}`,
        HttpStatus.BAD_REQUEST,
      );
    }
    return appointment;
  ...
  async todayFreeTimesOfStaff(name: string, date: string) {
    ...
    for (let staff of staffs) {
      let appointmentHours = (
        await this.getByStaffAndDate(dayDate, staff._id)
      ).map((a) => a.startTime);
      let emotyHours =
        appointmentHours.length > 0
          ? hours.filter((h) => !appointmentHours.includes(h))
          : hours;
  ...
  async getFreeStaffs(input: AppointmentInput): Promise<User[]> {
    ...
    const staffsOnLeave = await this.leaveService.checkIfStaffOnLeave(
      input.date,
      input.hours,
    );
    const staffsFree = staffsWithNoAppintment.filter(
      (s) => !staffsOnLeave.includes(s),
    );
    return staffsFree;
  }
  ...
  getDayDate() {
    ...
    let dayDate = `${tojalali.jy}-${tojalali.jm.toString().padStart(2, '0')}-${tojalali.jd.toString().padStart(2, '0')}`;
    return dayDate;
  }
}
