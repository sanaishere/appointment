...
export class AuthService {
    ...
    async patientSignUp(signUpInput:SignUpInput):Promise<User> {
        ...
        const patientExists=await this.userModel.findOne({where:{email:signUpInput.email}})
        ...
        else if(patientExists && !patientExists.roles.includes(UserRole.PATIENT)) {
            patientExists.caseNumber=caseNumber
            patientExists.roles.push(UserRole.PATIENT)
            return await patientExists.save()
        }
        else{
        const newPatient=await this.userModel.create({...signUpInput,caseNumber,roles:[UserRole.PATIENT]})
