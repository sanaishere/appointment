...
export class LeaveService {
    constructor(@InjectModel (LeaveRequest.name) private leaveRequestModel:Model<LeaveRequest>){}
    ...
    async requestToLeave(input:RequestInput,user:User){
            ...
            ({staff:user._id,hours,status:statusNow,createdDate,text:input.text,wantedDate:input.wantedDate})
            return leaveRequest.save()
        }

    async checkStaffRequests(status:string,staff:User):Promise<LeaveRequest[]> {
        const requests=await this.leaveRequestModel.find({status:status,staff:staff._id})
        return requests
    }
